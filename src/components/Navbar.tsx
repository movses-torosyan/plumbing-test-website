...
export function Navbar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-30 border-b border-white/15 bg-[#071a2f]/85 backdrop-blur-md transition-transform duration-300 ease-out motion-reduce:transition-none ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
      <nav aria-label="Main navigation" className="flex h-20 w-full items-center justify-between pl-5 pr-2 sm:pl-8 sm:pr-3 lg:pl-10 lg:pr-4">
        <Link href="#home" className="flex items-center gap-3" aria-label="Your Logo home">
